...
class ESLiftConvAttention(torch.nn.Module):
    def __init__(
        self,
        group: Group,
        in_channels: int,
        ...
        if True:
            self.conv_embed = GroupEquivariantPatchEmbedding(in_channels=self.in_channels, embed_dim=self.out_channels, patch_size=patch_size, stride=(patch_size//2) + 1,rotations=self.group.num_elements )
        else:
            # Define embeddings.
